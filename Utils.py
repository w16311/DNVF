...
def generate_grid3D_tensor(shape, factor = 2, sq = True):
    x_grid = torch.linspace(-1., 1., int(shape[0]/factor))
    y_grid = torch.linspace(-1., 1., int(shape[1]/factor))
    z_grid = torch.linspace(-1., 1., int(shape[2]/factor))
    ...
    if sq:
        grid = torch.stack([z_grid.flatten(), y_grid.flatten(), x_grid.flatten()], dim=1)
    else:
        grid = torch.stack([z_grid, y_grid, x_grid], dim=3)
    return grid
